...
class resource_lock:
    ...
    def lock(self, client_id, time_limit):
        ...
        """
        #if self.lock_count < self.num_locks:
...
class lock_pool:
    ...
    def lock(self, resource_id, client_id, time_limit):
        ...
        if self.__try_lock__():
            ...
            for i in range(self.num_resources):
                if resource_id <= i and resource_id >= 0:
                    resource = self.locks_array[resource_id - 1]
                    if resource.status() and client_id == resource.client_id:
                        return resource.lock(client_id, time_limit)
                    elif not resource.status():
                        return resource.lock(client_id, time_limit)
                else:
                    return False
    ...
    def ystats(self):
        ...
        for i in range(self.max_resource_lock):
            if not self.status(self.locks_array[i]):
                resource_locked += 1
    ...
    def nstats(self):
        ...
        for i in range(self.max_resource_lock):
            if self.status(self.locks_array[i]):
    ...
    def __repr__(self):
        ...
        for i in range(self.num_resources):
            ...
            if self.status(lock) == False:
                output += 'Resource <%s> BLOCKED by client <%d> until <%f>\n' % (
                    i, lock.client_id, time.ctime(lock.time_lock))
